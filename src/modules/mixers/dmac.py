...
class state_Mixer(nn.Module):
    def __init__(self, args):
        super(state_Mixer, self).__init__()
...
class state_obs_Mixer(nn.Module):
    def __init__(self, args):
        super(state_obs_Mixer, self).__init__()
        self.args = args
        self.n_agents = args.n_agents
        self.n_actions = args.n_actions
        self.state_dim = int(np.prod(args.state_shape))
        self.obs_dim = int(np.prod(args.obs_shape))
        self.all_obs_dim = self.obs_dim * args.n_agents
        self.action_dim = args.n_agents * self.n_actions
        self.state_action_dim = self.state_dim + self.action_dim
        self.state_obs_dim = self.state_dim + self.all_obs_dim
        self.unit_dim = 16
        self.n_head = args.n_head
        self.embed_dim = args.mixing_embed_dim
        self.attend_reg_coef = args.attend_reg_coef
        ...
        for i in range(self.n_head):  # multi-head attention
            ...
            self.keys.append(nn.Linear(self.state_dim, 1))
            self.agents_extractors.append(nn.Linear(self.state_dim, self.n_agents))
            self.action_extractors.append(nn.Linear(self.state_obs_dim, self.n_agents))
    ...
    def lambda_weight(self, states, obs):  # attention weights from Qplex
        states = states.reshape(-1, self.state_dim)
        obs = obs.reshape(-1, self.all_obs_dim)
        state_obs = th.cat([states, obs], dim=1)

        head_keys = [k_ext(states) for k_ext in self.keys]
        head_agents = [k_ext(states) for k_ext in self.agents_extractors]
        head_actions = [sel_ext(state_obs) for sel_ext in self.action_extractors]
    ...
    def get_q_coeff(self, states, obs):
        weight, attend_mag_regs = self.weight(states)
        weight = weight.reshape(-1, self.n_agents) + 1e-10
        lambda_weight = self.lambda_weight(states, obs)
        coeff = weight * lambda_weight
        return coeff
